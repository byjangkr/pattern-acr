...
para.maxEpoch = 10; % the number of epoch for regression
para.levelScale = 5;
para.set = [1 2 3];

para.selReg = 1; % select regression machine 
...
%para.fid = [1 2 3 4 6 9 10 11 13 14 15 17 19 20]; % core (high corr)
para.task_id = [2 3 4 5]; % using task index for correlation between score of raters and each features

%% Make est data
para.tranneg = 0; % translate from negative feat to positive feat
...
for itask=para.corr_task_id
    corr_data = [corr_data; fludsf_data(fludsf_data(:,3)==itask,:)];
end
for iSet = para.set
